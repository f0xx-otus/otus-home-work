...
func generate(path string, validateMap map[string][]field) error {
...
`
	...
	for _, s := range validateMap {
		...
		fmt.Println("template struct", tempStruct[0])
		fmt.Printf("%T\n", tempStruct[0])

		err = t.ExecuteTemplate(file, "validate", tempStruct[0])
		//fmt.Println("s is", s)
		if err != nil {
			return err
		}
